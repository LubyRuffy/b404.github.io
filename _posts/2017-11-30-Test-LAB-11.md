...
使用dirbuster扫描web文件目录,扫描到多个登陆界面，尝试弱口令都无果。后`http://192.168.11.3/_admin/login.php`,尝试弱口令
...
登陆ssh之后，发现cd命令不能执行，`echo $0`查看shell是rbash(受限的bash命令)，就使用`python -c 'import pty; pty.spawn("/bin/sh")'`将shell环境切换为bash：
...
> 该perl脚本从`/build/log`读取文件，将文件内容作为变量读取，使用print命令写入到`/tmp/testlog`，然后从`/build/log`文件目录删除文件(即取消文件链接)。所以将需查看文件的文件名写入到`/build/log`日志文件中，就可以在`tmp/testlog`读取
